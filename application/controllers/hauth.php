...
class HAuth extends CI_Controller {
        ...
        try {
            ...
            if ($this->hybridauthlib->providerEnabled($provider)) {
                ...
                if ($service->isUserConnected()) {
                    ...
                    $this->verifylogin($user_profile->emailVerified);
            ...
            switch ($e->getCode()) {
                ...
                case 5 : log_message('debug', 'controllers.HAuth.login: Authentification failed. The user has canceled the authentication or the provider refused the connection.');
                    ...
                    redirect("/login/index/User has cancelled the authentication or the provider refused the connection.");
                    break;
    ...
    private function verifylogin($email) {

        if ($email) {
            $this->load->model('users_model', 'users');
            $user = $this->users->get_by_email($email);
            if (empty($user) || count($user) == 0) {
                $message = "No registred account found";
