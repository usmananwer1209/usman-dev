	...
	$.Baraja.prototype = {
		...
		_add : function( $elems ) {
			...
			$elems.css( 'transform', 'scale(1.8) translate(200px) rotate(15deg)' ).reverse().each( function( i ) {
				...
				self._setTransition( $el, 'all', 1, 'ease-out', i * 10 );
		...
		// remove elements from the deck
		_remove : function( $elems ) {

			//var self = this, elemsCount = $elems.length, cnt = 0;
			//console.log($elems);
			//logError($elems);

			//$($elems).css('display', 'none');
			if($($elems).length) {
				var elmt_zIndex = parseInt($($elems).css('zIndex'));
				$($elems).remove();

				//we get all elements with a bigger z-index and reduce it
				this.$el.children( 'li' ).each(function(i){
					var curr_zIndex = parseInt($(this).css('zIndex'));
					if(curr_zIndex > elmt_zIndex)
						$(this).css('zIndex', curr_zIndex-1);
				});
		...
		_dispatch : function( action, args ) {
			...
			if((action === this._add || action === this._remove) && this.isAnimating) {
				if(action === this._add){
		...
		remove : function ( $elems ) {
			this._dispatch( this._remove, $elems );
			//this._remove($elems);
